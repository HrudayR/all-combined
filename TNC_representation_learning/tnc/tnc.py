...
from models import RnnEncoder, WFEncoder
from utils import plot_distribution, track_encoding
...
def learn_encoder(x, encoder, window_size, w, lr=0.001, decay=0.005, mc_sample_size=20,
    ...
    for cv in range(n_cross_val):
        if 'waveform' in path:
            ...
            batch_size = 10
        elif 'mendeley' in path:
            encoder = RnnEncoder(hidden_size=100, in_channel=2, encoding_size=10, device=device)
            batch_size = 10
...
def main(is_train, data_type, cv, w, cont):
    ...
    if data_type == 'simulation':
        window_size = 50
        encoder = RnnEncoder(hidden_size=100, in_channel=3, encoding_size=10, device=device)
        path = '/home/hruday/PycharmProjects/TNC_representation_learning/TNC_representation_learning/data/simulated_data/'
    ...
    if data_type == 'mendeley':
        window_size = 10
        encoder = RnnEncoder(hidden_size=100, in_channel=2, encoding_size=10, device=device)
        path = '/home/hruday/PycharmProjects/TNC_representation_learning/TNC_representation_learning/data/mendeley_data/'

        if is_train:
            with open(os.path.join(path, 'x_train.pkl'), 'rb') as f:
                x = pickle.load(f)
            learn_encoder(x, encoder, w=w, lr=1e-3, decay=1e-5, window_size=window_size, n_epochs=200,
                          mc_sample_size=40, path='mendeley', device=device, augmentation=5, n_cross_val=cv)
            ...
            encoder.load_state_dict(checkpoint['encoder_state_dict'])
            encoder = encoder.to(device)
            track_encoding(x_test[0,:,:], y_test[0,:], encoder, window_size, 'mendeley')


            # x_test = TNCDataset(x=torch.Tensor(x_test), mc_sample_size=40, window_size=window_size, augmentation=5, adf=True)
            # test_loader = data.DataLoader(x_test, batch_size=10, shuffle=False, num_workers=3)
            # disc_model = Discriminator(encoder.encoding_size, device)
            # params = list(disc_model.parameters()) + list(encoder.parameters())
            # optimizer = torch.optim.Adam(params, lr=1e-3)
            # epoch_loss, epoch_acc = epoch_run(test_loader, disc_model, encoder, train=False, w=w, device=device)
    ...
    if data_type == 'har':
        window_size = 4
        path = '/home/hruday/PycharmProjects/TNC_representation_learning/TNC_representation_learning/data/HAR_data/'
        encoder = RnnEncoder(hidden_size=100, in_channel=561, encoding_size=10, device=device)
